...
event["account_id"] = account.get_caller_identity()['Account']
event["account_detail"] = account.get_caller_identity()['Account']

# Initialize and Connect to the AWS EC2 Service
try:
    ec2_client = boto3.client("ec2")
    # cloud = boto3.client('cloudfront')
...
# response33 = cloud.get_distribution_config()
# print(response33)
# time.sleep(555)
...
def lambda_handler(event, context):
    ...
    try:
        ...
        for i in range(len(parent_list)):
            ...
            # push_to_gateway(
            #     'localhost:9091', job=account_id, registry=registry
            # )
